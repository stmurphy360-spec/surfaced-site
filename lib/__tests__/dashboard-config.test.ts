...
/** Test fixture helper — allows extra fields not in ConfigResponse */
function mockConfig(data: Record<string, unknown>): ConfigResponse {
  return data as unknown as ConfigResponse
}
...
test('returns array with one ProductLineCard for a single product line', () => {
  const result = parseDashboardConfig(mockConfig({
    brand_name: 'Acme',
    competitors: { loans: ['X'] },
